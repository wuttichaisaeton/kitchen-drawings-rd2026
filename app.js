...
async function _kdOpen3D(code, opts) {
  ...
  const STYLE = `<style>
    ...
    /* เอ๋ 2026-06-24: bigger explode slider thumb (44px hit row / 40px thumb) — was
       28px, still hard to tap. margin-top centres the 40px thumb on the 12px track. */
    .kd3d-modal .kd3d-explodebar input[type=range]{flex:1 1 auto;min-width:0;-webkit-appearance:none;appearance:none;background:transparent;height:44px;cursor:pointer;touch-action:manipulation;margin:0}
    .kd3d-modal .kd3d-explodebar input[type=range]::-webkit-slider-runnable-track{height:12px;border-radius:6px;background:#2b3340}
    .kd3d-modal .kd3d-explodebar input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:40px;height:40px;margin-top:-14px;border-radius:50%;background:#f2a93b;border:3px solid #fff;box-shadow:0 2px 7px rgba(0,0,0,.55);cursor:pointer}
    ...
    /* เอ๋ 2026-06-24: close ✕ moved to a floating button bottom-RIGHT (was top-right
       header, overlapped the W/D/H dims on mobile). Dark circle, mirrors the fit
       button bottom-left. !important beats the theme button override. */
    .kd3d-modal .kd3d-viewer .kd3d-close-float{position:absolute !important;right:10px;bottom:10px;top:auto;left:auto;z-index:5;display:inline-flex !important;align-items:center;justify-content:center;width:42px !important;height:42px !important;min-width:42px;border-radius:50% !important;background:#161c25 !important;color:#fff !important;border:1px solid #2b3340 !important;font-size:19px;line-height:1;cursor:pointer;padding:0 !important;margin:0 !important;box-shadow:0 2px 9px rgba(0,0,0,.4) !important;transition:background .12s ease,transform .12s ease}
    .kd3d-modal .kd3d-viewer .kd3d-close-float:hover{background:#0b0f14 !important;transform:scale(1.08)}
    .kd3d-modal .kd3d-viewer .kd3d-close-float:active{transform:scale(.94)}
  ...
  body.innerHTML = `
      ...
      <div class="kd3d-viewer" style="position:relative">
        ...
        <button class="kdstock-close kd3d-close-float" aria-label="Close">✕</button>
        <model-viewer
  ...
  // room). เอ๋ 2026-06-24 "zoom out ออกมาหน่อย" → 1.4→1.7→2.1→2.6 (kept asking
  // for more). Shared by BOTH fit paths (_fitCamera + _fitVisibleWorld) so the
  // auto-fit on open and the manual fit button frame at the same distance.
  const _FIT_MARGIN = 2.6;
  ...
  };
  {
    ...
    if (fitBtn) fitBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      // click pulse (เอ๋ "ทำ effect") — re-trigger reliably on repeated clicks.
      fitBtn.classList.remove('kd3d-fit-go'); void fitBtn.offsetWidth; fitBtn.classList.add('kd3d-fit-go');
  ...
  const _buildExplodeLabels = async () => {
    ...
    if (!explodeUnits.length || mode !== 'explode') return;
    ...
    // XXXXXX-WWWHHH form (drops raw GLB hardware names like legs/hinges). A code
    // qualifies if it has ≥1 visible non-orphan body.
    ...
    // EQUAL split: first half (alphabetical) → LEFT column, the rest → RIGHT.
    const half = Math.ceil(codes.length / 2);
    const cols = { L: codes.slice(0, half), R: codes.slice(half) };

    // ── Overlay DOM (STATIC legend) ──────────────────────────────────────
    ...
    (body.querySelector('.kd3d-viewer') || body).appendChild(_ovlRoot);

    _ovlRows = [];
    const asmSet = _assembledCodeSet();   // ticked-assembled codes → green ✓
    ...
    const _mkRow = (code, side, topPct) => {
      ...
      if (asmSet.has(code)) { const ck = document.createElement('span'); ck.className = 'kd3d-ovl-check'; ck.textContent = '✓'; row.appendChild(ck); }
      // "N x CODE" — qty bold, code regular.
      const qtyEl = document.createElement('span'); qtyEl.className = 'kd3d-ovl-qty'; qtyEl.textContent = qty;
      const codeEl = document.createElement('span'); codeEl.className = 'kd3d-ovl-code'; codeEl.textContent = ' x ' + code;
      row.appendChild(qtyEl); row.appendChild(codeEl);
      // Click → isolate this code's parts + zoom-fit; click again restores all.
      row.addEventListener('click', (e) => {
        e.stopPropagation();
        _poppedCode = (_poppedCode === code) ? null : code;
        _applyRowSelection();
        applyExplode(explodePct);
        requestAnimationFrame(() => _fitVisibleWorld());
      });
      _ovlRoot.appendChild(row);
      _ovlRows.push({ code, qty, side, rowEl: row });
    };
    // Each column listed top→bottom at FIXED % positions (evenly spread 6%→86%).
    // STATIC — no projection, no camera-follow; labels don't run up/down on orbit.
    for (const side of ['L', 'R']) {
      const list = cols[side], n = list.length;
      list.forEach((code, i) => _mkRow(code, side, n > 1 ? (6 + i * (80 / (n - 1))) : 46));
    }
    // Hidden until explode > 5% (mirrors the old gating).
    _ovlRoot.style.display = (explodePct > 5) ? '' : 'none';
    console.info('[kd3d] overlay built —', _ovlRows.length, 'static labels (L', cols.L.length, '/ R', cols.R.length, ')');
  };
  ...
  const _syncExplodeHotspots = () => {
    // เอ๋ 2026-06-24: explode labels are now a STATIC alphabetical legend (two
    // equal columns, fixed positions — no hotspots, no leaders, no camera-follow).
  ...
  if (typeof navigator !== 'undefined' && navigator.hid) {
    ...
    // เอ๋ 2026-06-24: the bottom-left "3D" (3Dconnexion connect) button is removed
    // — NOT appended to the DOM. The HID auto-connect below still works silently
    // if a SpaceMouse is already paired; there's simply no visible button.
...
function renderProjectsHome() {
  ...
  // (Removed: generic "📋 Copy Assembler Link" header bar — เอ๋ 2026-06-22
  // "เอาปุ่ม Copy Assembler Link ออก". The ?asm deep-link still works for
  // anyone holding it; only the in-app copy button is gone.)
