...
## Amendment — 2026-05-30 (post-ship, per เอ๋)

The grain warning shipped, then เอ๋ refined it. The sections below describe the
**original** design; the current behaviour is:

- **Grain → NO banner.** The amber grain banner (decision 3, FORK B) was removed
  (commit `51319ba`). Grain warns ONLY via the per-row amber ring marker in the
  grain cell.
- **Marker flags DIRECTIONAL grain, not default-ANY (FORK B inverted).** เอ๋:
  "บีเค grain ไปทางแนวนอน … อันนี้ที่ต้องเตือน". A part whose grain is `H` (─) or
  `V` (│) gets the marker — the worker must lay the grain the right way. `ANY`
  (✱, rotates freely) is NOT flagged. Predicate renamed
  `_isGrainUncertain` → `_isGrainDirectional` (`p.selected && !p.manual &&
  (grain === 'H' || grain === 'V')`); the `grainExplicit` provenance flag is
  removed (no longer needed). Commit `8d8f3e6`.
- **Size-mismatch tolerance widened 10mm → 25mm** (`const TOL = 25`, commit
  `2e47251`) so panel-vs-channel + tier-rounded codes don't over-flag.
